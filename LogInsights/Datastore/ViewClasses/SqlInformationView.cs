        ...
        public override void ExportView(string key)
        {
            ...
            try
            {
                ...
                if (key == BaseKey)
                {
                    ...
                    uc[0].AddItemRow("number",
                        new string[] {"number of sql session found", dsref.numberOfSqlSessions.ToString()});
                    uc[0].AddItemRow("isSuspicious",
                        new string[]
                        {
                            "is suspicious",
                            dsref.isSuspicious
                                ? "yes; please check some of the sql session marked"
                                : "seems to be all fine"
                        });
                    uc[0].AddItemRow("threshold_suspicious_duration_SqlCommand_msec",
                        new string[]
                        {
                            "defined config value: threshold_suspicious_duration_SqlCommand_msec",
                            dsref.ThresholdSuspiciousDurationSqlCommandMsec.ToString()
                        });
                    uc[0].AddItemRow("threshold_suspicious_duration_SqlTransaction_min",
                        new string[]
                        {
                            "defined config value: threshold_suspicious_duration_SqlTransaction_sec",
                            dsref.ThresholdSuspiciousDurationSqlTransactionSec.ToString()
                        });
                ...
                if (key == BaseKey + "/sessions")
                {
                    ...
                    uc[0].SetupHeaders(new string[]
                    {
                        "Session id", "Is suspicious", "Session start", "Session ends", "Session duration",
                    ...
                    foreach (var s in dsref.SqlSessions)
                        uc[0].AddItemRow(s.uuid, new string[]
                        {
                            ...
                            s.transactions.Count == 0
                                ? "-"
                                : s.transactions.OrderByDescending(t => t.durationMin).First().GetLabel(),
                            s.longRunningStatements.Count.ToString(),
                            s.longRunningStatements.Count == 0
                                ? "-"
                                : s.longRunningStatements.OrderByDescending(t => t.durationMsec).First().GetLabel()
                        });

                    uc[0].ItemClicked += new ListViewItemSelectionChangedEventHandler(
                ...
                if (key.StartsWith(BaseKey + "/sessions/"))
                {
                    ...
                    if (sess != null)
                    {
                        ...
                        {
                            "Timestamp", "Locator", "Duration [ms]", "Logger id", "Command", "Involved tables",
                            "Statement text"
                        });

                        uc[2].SetupCaption("transacount list");
                        uc[2].SetupHeaders(new string[]
                            {"Start", "End", "Duration", "Start locator", "End locator", "State", "Logger id"});

                        //fill lists with data
                        uc[0].AddItemRow("firstOccurrence",
                            new string[] {"first occurrence", sess.dtTimestampStart.ToString("G")});
                        uc[0].AddItemRow("lastOccurrence",
                            new string[] {"last occurrence", sess.dtTimestampEnd.ToString("G")});
                        uc[0].AddItemRow("isSuspicious", new string[] {"is suspicious", sess.isSuspicious.ToString()},
                            "", GetBgColor(!sess.isSuspicious));
                        ...
                        uc[1].ItemClicked += new ListViewItemSelectionChangedEventHandler(
                            (object o, ListViewItemSelectionChangedEventArgs args) =>
                            {
                                ...
                                {
                                    string k = args.Item.Name;
                                    var statement = dsref.SqlSessions.FirstOrDefault(t => t.uuid == uuid)
                                        .longRunningStatements.FirstOrDefault(st => st.uuid == k);
                        ...
                        uc[2].ItemClicked += new ListViewItemSelectionChangedEventHandler(
                            (object o, ListViewItemSelectionChangedEventArgs args) =>
                            {
                                try
                                {
                                    string k = args.Item.Name;
                                    var tra = dsref.SqlSessions.FirstOrDefault(t => t.uuid == uuid).transactions
                                        .FirstOrDefault(st => st.uuid == k);
